...
* Velocity sensitivity and modulation, I want this to be a player’s synth. One velocity slot should be assigned to amp level by default, and the other slot should be on “None”.  
* Computer friendly UI  
* Midi compatibility and virtual keyboard, with midi assign for configuring midi keyboards
...
1. The lfo and velocity target selectors need dropdowns, or it’s difficult to scroll through  
2. Tuning is a bit hard to put a finger on, by how much is the tuning going up and down?  
3. The oscillators and the distortion sound very digital, maybe we should try to make the distortion sound more organic in case the user wants that kind of feel  
4. Release and LFo Rate controls go up too quickly, they need to be exponential in nature to give us finer control for smaller release values  
5. Needs midi assign  
6. Some of the instrument sections wobble side-to-side when adjusting faders, such as the filter section  
7. Needs pitch-bend
